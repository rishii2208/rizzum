...
import { compileLatex, fetchAtsScore, optimizeResume } from "./lib/api";
...
const Panel = ({ title, children, actions }) => (<section className="space-y-3 rounded-2xl border border-white/10 bg-slate-900/60 p-4 shadow-2xl shadow-black/50">
...
function App() {
    ...
    const compile = useCallback(async (latex) => {
        try {
            if (!latex.trim())
                return;
            setIsCompiling(true);
    ...
    const runAtsScore = useCallback(async (payload) => {
        try {
      ...
      <header className="space-y-3 border-b border-white/10 bg-slate-950/90 px-6 py-8 backdrop-blur">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleDownload} disabled={!pdfBase64 || isCompiling} className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-white transition hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-40">
              Download PDF
            </button>
          </div>
        </div>
      ...
      <main className="grid gap-6 p-6 lg:grid-cols-2">
        <div className="space-y-6">
          ...
          <Panel title="Original Resume LaTeX">
            <Editor height="420px" defaultLanguage="latex" theme="vs-dark" options={editorOptions} value={resume} onChange={(val) => setResume(val || "")}/>
          </Panel>

          <div className="flex justify-end">
            <button onClick={handleOptimize} disabled={isOptimizing} className="flex items-center gap-2 rounded-full bg-brand-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-brand-900/50 transition hover:bg-brand-500 disabled:cursor-not-allowed disabled:opacity-50">
        ...
        <div className="space-y-6">
          <Panel title="Optimized LaTeX">
            <Editor height="360px" defaultLanguage="latex" theme="vs-dark" options={editorOptions} value={optimizedLatex} onChange={(value) => setOptimizedLatex(value || "")}/>
          </Panel>

          <Panel title="Live PDF Preview" actions={<span className="text-xs text-white/60">
                {isCompiling ? "Compiling…" : pdfBase64 ? "Up to date" : "Waiting for LaTeX"}
              </span>}>
            <div className="h-[420px] overflow-hidden rounded-xl border border-white/10 bg-black/40">
              {pdfDataUrl ? (<iframe title="resume-preview" src={pdfDataUrl} className="h-full w-full"/>) : (<div className="flex h-full items-center justify-center text-white/40">PDF preview pending</div>)}
            </div>
          </Panel>

          {atsScore && (<Panel title="ATS Insights">
              <div className="space-y-4 text-sm text-white/80">
                <div>
                  <p className="text-xs uppercase tracking-wide text-white/50">Top matched keywords</p>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {atsScore.matchedKeywords.map((keyword) => (<span key={keyword} className="rounded-full bg-emerald-500/10 px-3 py-1 text-emerald-300">
                        {keyword}
                      </span>))}
                ...
                <div>
                  <p className="text-xs uppercase tracking-wide text-white/50">High-priority gaps</p>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {atsScore.missingKeywords.map((keyword) => (<span key={keyword} className="rounded-full bg-rose-500/10 px-3 py-1 text-rose-300">
                        {keyword}
                      </span>))}
